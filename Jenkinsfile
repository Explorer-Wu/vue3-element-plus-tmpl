...
pipeline {
    agent {label 'jenkins-vue3-element-test'}
    ...
    stages {
        stage('Init'){
            steps {
                script{
                    // 【测试环境】变量
                    env.INGRESS_HOST='admin-tmpl-test.vue3test.cn'
                    ...
                    // 【开发环境】变量
                    if( "${BRANCH_NAME}"== "dev"){
                        env.INGRESS_HOST='admin-tmpl-dev.vue3test.cn'
                        env.INGRESS_TLS_SECRET="vue3test-tls-secret"
                        env.REPLICAS="1"
